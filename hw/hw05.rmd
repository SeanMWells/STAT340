...
We would generally expect to make (alpha * m) number of type I errors.   
...
family-wise error rate = P(at least one error) = 1 - P(0 errors)  
= 1 - (0.95)^m   
...
We expect the FWER to be lower for positively correlated tests compared to that of independent tests. Seeing as the two tests will produce essentially the same p-value, the odds of at least one type I error should simply be equal to alpha.     
...
We expect the FWER to be greater for negatively correlated tests compared to that of independent tests. Because we know that all of these test are true, the odds of receiving a relatively high p-value (and thus by extension another low and statistically significant p-value) becomes much higher due to the existence of a negative correlation between the two tests.    
...
for (i in 1:100){
  data = X[,i]
  pvs[i] = t.test(data)$p.value
}
hist(pvs, main="Histogram of p-values", xlab="p-value")
```  
...
sum(pvs < 0.05)
```
We would reject 4 of the null hypotheses.
...
sum(pvs < (0.05/100))
```
We wouldn't reject any of the null hypotheses when controlling the FWER at level 0.05. 
...
```{r} 
idxs = order(pvs)[1:10]
cherry_picked = pvs[c(idxs)]
sum(cherry_picked < (0.05/10))
```
From our cherry-picked sample of the 10 fund managers with the lowest p-values, if we control the FWER at level 0.05 we would reject the null hypothesis for one test. 
...
for( i in 1:200 ) {
  resample = sample(mule_kicks$deaths, 280, replace=TRUE)
  replicates[i] = mean(resample)
}
sd_boot = sd(replicates)
c(lambdahat-1.96*sd_boot, lambdahat+1.96*sd_boot)
```
We are 95 percent confident that the true lambda parameter of this Poisson distribution is between 0.59 and 0.81. 
...
mule_kick_bootstrap <- function( B ) {
  ...
  for( i in 1:B ) {
    ...
    for( idx in 1:200 ) {
      resample = sample(mule_kicks$deaths, 280, replace=TRUE)
      replicates[idx] = mean(resample)
    }
    sd_hats[i] = sd(replicates)
  }
  sd_hat = mean(sd_hats)
  return(c(lambdahat-1.96*sd_hat, lambdahat+1.96*sd_hat))
}
```
...
Notice that as the number of bootstrap simulations increases, the magnitude of the confidence interval becomes smaller; the interval narrows. The connotations that this implies could be that choosing a number of bootstrap samples B that is too low would lead to a large confidence interval that doesn't actually provide useful information about the data; leading to more type II errors as samples that would lie outside of a reasonable confidence interval instead are found within due to the bootstrap. On the other hand, a bootstrap that uses too many samples B could increase the number of type I errors as the interval becomes so small that samples that actually do follow the null hypothesis are rejected. It should also be considered that a large number of bootstrap samples could take a long/excessive computational time. 
